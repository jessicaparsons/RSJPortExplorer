...
class GalleryViewModel: ObservableObject {
    ...
    @Published var gridLayout: [GridItem] = Array(repeating: .init(.flexible(), spacing: Constants.verticalSpacing / 2), count: 2)
    ...
    //Grid Layout Options
    func gridSwitch() {
        //print("Before gridSwitch: \(gridLayout.count) columns")
        withAnimation(.easeIn) {
            if horizontalSizeClass == .compact {
                // Cycle between 1, 2, 3 columns for compact size class
                let columnCount = (gridLayout.count % 3) + 1
                gridLayout = Array(repeating: .init(.flexible(), spacing: Constants.verticalSpacing / 2), count: columnCount)
           ...
           // print("After gridSwitch: \(gridLayout.count) columns")
        }
